import { getFakeTransactions, getFakeCategories, getFakeBudgets, getFakeUsers, saveFakeBudgets } from '../api/usersApi.js';
...
function uid() {
    return 'budget_' + Date.now() + Math.random().toString(36).substr(2, 5);
}
...
function renderBudgets() {
    ...
    const title = document.getElementById('budgetPageTitle');
    const subtitle = document.getElementById('budgetPageSubtitle');
    if (CU.role === 'admin') {
        if (title) title.textContent = 'Tous les budgets';
        if (subtitle) subtitle.textContent = '';
    } else {
        if (title) title.textContent = 'Mes budgets';
        if (subtitle) subtitle.textContent = 'Vous gérez uniquement vos propres budgets';
    }
    ...
    document.getElementById('budgetList').innerHTML = budgets.length ? budgets.map(b => {
        ...
        const scope = CU.role === 'admin' ? 'admin-scope' : 'user-scope';
        return `<div class="section ${scope}" style="margin-bottom:16px">
...
function openBudgetModal() {
    ...
    if (CU.role === 'admin') {
        ...
        document.getElementById('budgetUser').innerHTML = getFakeUsers().map(u => `<option value="${u.id}">${u.name}</option>`).join('');
    } else {
...
function deleteBudget(id) {
    ...
    saveFakeBudgets(getFakeBudgets().filter(b => b.id !== id));
    renderBudgets();
}
...
window.toggleBudgetDetails = toggleBudgetDetails;
